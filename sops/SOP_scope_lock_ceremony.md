...
The Critic perspective MUST be invoked via the Agent tool with a sealed prompt and a prompt_hash recorded. **The sealed prompt SHALL open with `Today's date: YYYY-MM-DD.` per `docs/CONVENTION_delegated_prompt_date.md` (gh#1812 — dateless delegates flag valid dates as fabricated)** in `planning/triad_findings.jsonl`. Label-only verification is insufficient (per F-CRITIC-009 v3.16; AM1 v1.1.0 amendment of v3.17 lock plan). The prompt_hash is the structural proof that distributed Critic actually executed.
...
- [ ] **G1.PORTFOLIO: Initiative-portfolio grooming pass + deferral ingest (NEW v1.7.0 — D-IG-3 + gh#1536)** — two halves, both MANDATORY at Gate 1:
  ...
  3. **Re-verify at INGEST, not only at Tier placement (v1.7.2 — C-27-14 / F-2026-07-16-D)**: every carried/ingested row's source claim (issue state, plan header, artifact existence, "shipped" assertions) is re-verified AT THE MOMENT it enters the new cycle's inventory — a PROCEED disposition without a same-day source check is not a disposition. Evidence base: 3/16 of v3.26's carry set was obsolete on arrival (19%); v3.27 caught 3 more pre-build (#1823 CLOSED member, #1889 spec-half shipped, C-27-03 built-and-propagated before its build gate opened). The verification record (what was checked, when) lands in the ingest note per row.
