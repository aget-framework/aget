...
- `template-document-processor-AGET` is structurally deficient (missing manifest/identity/5D dirs/governance; `instance_type='AGET'`) — a pre-existing condition that prior releases also shipped. It carries the v3.21.0 version tag for fleet-set consistency; repair is tracked at gmelli/aget-aget#1593.
