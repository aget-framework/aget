...
- **`/aget-create-initiative`** in canonical core — STRICT initiative scaffolding (D71). **⚠️ Errata (2026-06-07, gh#1461):** the verb-pair is **half-shipped** — the producer `/aget-propose-initiative` skill and `SOP_initiative.md` are absent from canonical at this tag, making the STRICT route **non-functional**; it is **gated (not enforced)** pending the producer half. See gh#1461.
