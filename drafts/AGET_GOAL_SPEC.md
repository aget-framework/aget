...
| Purpose / North Star | C957 / C966 | The permanent "why"; the apex carrier (distinct from a product KPI). |
| Theme | C958 | An organizing band over Goals — corresponds to an Initiative; not a Goal. |
| Goal type | C963 | KAOS typing: **Achieve** (one-shot end-state — the typical committed Goal), **Maintain** (a steady-state the agent regulates — MP#12 loops), **Soft** (a quality optimized, not satisfied). |
...
A Goal SHALL be typed as one of {Achieve, Maintain, Soft} (KAOS, C963). Maintenance loops surfaced by MP#12 SHALL be representable as **Maintain** Goals (C965 — closing the currently type-less maintenance-goal gap).
...
- `id` — Goal Identifier (unique within instance; scheme per the grounding-gap concept, pending)
...
|------|---------|-------------|
| **Goal Identifier** | REQ-GOAL-4, CAP-GOAL-009 | needs a concept (id scheme + uniqueness) |
| **Goal Value** (potential value) | REQ-GOAL-5, CAP-GOAL-010 | the L1093 gap — value-rubric family never covers the goal tier; pairs with `RUBRIC_goal_value` |
| **Commitment Tag** (committed/aspirational) | REQ-GOAL-5, CAP-GOAL-010 | C929 covers the tier, not the tag values |
