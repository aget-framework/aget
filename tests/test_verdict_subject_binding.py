...
def test_unreached_voice_subject_has_all_four_limbs_and_limit(tmp_path):
 root=tmp_path/"r";root.mkdir();d=root/"x";d.mkdir();d.chmod(0o400)
 try:
  r=voice.assess(root,[])
  c=r["classification"]; assert c["subject_bound"] and not c["subject_reached"]
...
def test_bound_observation_is_not_withheld(tmp_path):
 root=tmp_path/"r";(root/"knowledge/voice").mkdir(parents=True)
 (root/"knowledge/voice/README.md").write_text("voice")
 d=root/"consumer";d.mkdir();(d/"x.md").write_text("knowledge/voice/README.md\nSpecification Evidence Bank Enforcement")
 r=voice.assess(root,[])
...
def test_dangling_observation_survives_as_definite_finding(tmp_path):
 root=tmp_path/"r";root.mkdir();(root/"x.md").write_text("VOICE.md")
 r=voice.assess(root,[])
