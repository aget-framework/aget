...
from warranted_verdict import (  # noqa: E402
    DEFINITE, INDEFINITE, Verdict, UnwarrantedVerdict, worst,
)
...
def test_worst_does_not_launder_an_indefinite_state_into_a_pass():
    assert worst([Verdict.passed(warrant="ok"),
                  Verdict.unknown(limit="not readable")]) == "UNKNOWN"
    assert worst([Verdict.passed(warrant="ok"),
                  Verdict.unavailable(limit="absent")]) == "UNAVAILABLE"


def test_an_inert_check_neither_fails_nor_passes_on_its_own():
    assert worst([Verdict.inert(limit="precondition absent")]) == "INERT"
    assert worst([Verdict.passed(warrant="ok"),
                  Verdict.inert(limit="did not fire")]) == "PASS"
...
def test_CLASS_an_unresolvable_evidence_path_yields_no_claim_about_the_evidence(tmp_path):
    ...
    pkt = {"cap_su": 10, "pool_allows_two_l3": True, "candidates": [
        {"id": "A", "disposition": "SELECT", "tier": "T1", "su": 7, "predicted_v1": "L3",
         "class": "capability", "source": {"path": "/etc/hosts", "pattern": "localhost"}}]}
    res = fc = fvg.forecast(pkt, None, tmp_path)
...
def test_CLASS_an_ungoverned_region_yields_no_conformance_claim(tmp_path):
    ...
    m = tmp_path / "m.json"
    m.write_text(json.dumps({"paths": [{"pattern": "data/*.toml", "class": "config",
                                        "tier": "record"}]}))
...
def test_CLASS_an_unrunnable_verification_yields_no_verification_claim(tmp_path):
    ...
    subprocess.run(["git", "-C", str(r), "-c", "user.name=t", "-c", "user.email=t@t",
                    "-c", "commit.gpgsign=false", "commit", "-q", "-m", "c"], check=True)
    ...
    rp.write_text(json.dumps(rec)); mp.write_text(json.dumps(rec))
