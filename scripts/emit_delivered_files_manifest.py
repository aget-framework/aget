...
def template_shipped_paths(version):
    ...
    files. Found by the originating supervisor seat's independent migration-prep audit, not
    by this producer.
...
def emit(version, ref):
    ...
    lines += [
        ...
        "  consumer (the originating supervisor seat) before it ran G0.2, not by the producer.",
    ]
