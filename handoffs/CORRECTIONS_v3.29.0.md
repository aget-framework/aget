...
| 4 | The manager's three-axis tag checker treated every path in every M-row detection clause as required at all 14 tags, ignoring seat applicability and the declared canonical distribution point. | The checker now honors structured `tagged_origins` and checks `distribution_point` paths for scoped rows; v3.29 declares canonical plus advisor-exemplar coverage per the locked release plan. | No payload copy is implied for non-applicable template tags; receiving seats still copy the declared canonical distribution point before running applicable detections. |
