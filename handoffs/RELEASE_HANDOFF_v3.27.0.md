...
| **remote fleet supervisor (FIRST REMOTE ROW)** | 3.27.0 | 2026-07-18 | Rung-4 self-reported with behavioral detail: 6-probe smoke + own test suite vs pre-migration baseline (byte-identical FAILED/ERROR sets, not count-matching) + dual-basename parity; contract sourced from `origin/main` per the corrections instructions. Caveat recorded honestly: cross-machine — commits and working trees not independently verifiable by this seat; stamped on tracker-filed evidence | gmelli/aget-aget#1957 (`[XP]` experience report — first remote-fleet confirmation ever filed; the confirm-back contract's first exercise) |
| *(post-pilot)* full remote fleet | 3.27.0 | 2026-07-18 | 6/6 used seats same-day at the Rung-4 bar (1 seat standing-excluded by that fleet's own ruling); 36/36 probe-slots PASS/correct-N-A/correct-SKIP; zero test regressions; function-preserving merges on 3 customized seats with zero local capability lost | gmelli/aget-aget#1957 |

**Phase 7.6 delivery record (honest)**: the formal Option-B notification relay was overtaken by events — the remote fleet discovered the release passively same-day and initially prepared from tag-pinned reads (the L1214 incident); corrective content reached them via principal-carried advice plus the hardened `main` surfaces (CORRECTIONS row 7), which their own session verified first-party before Wave 0. Confirmation arrived via #1957 (above), satisfying the "behavioral results, not disk receipts" bar.
