...
6. **Customized-base-script caution** (added 2026-07-05, from the second fleet's Wave-0): "overwrite" is safe only for scripts you never patched. `health_check.py` and `study_topic.py` currently ship **no `_ext` hook**, so any local checks you added live in the base file — a blind overwrite deletes them (observed: 8 local health-check invariants at risk in one supervisor). Before overwriting, diff local vs source at function level; local-only defs mean you need a **function-preserving merge**, not an overwrite. And **hold the version bump until every script carries the new substance** (conformance-then-bump) — stamping the version while scripts are unmerged is the symbol-vs-substance trap.
