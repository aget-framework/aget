...
| R-REL-032-07a | ubiquitous | The validator SHALL verify, post-release, that the **aget core** (`aget-framework/aget`) GitHub Release body content meets **depth-driven** norms: floor 1000 bytes for non-trivial releases (sleeping-CAPs disclosure or substantive theme may justify above-norm without WARN); soft cap 3000 for non-breaking minors (up to ~9000 acceptable per RELEASE_BODY_TEMPLATE_core.md exception class for sleeping-CAPs disclosure depth); body SHALL be L909-sanitized; body SHOULD source from CHANGELOG entry per SOP_release_process v1.32 Phase 6.4.5.3. | v3.16.0 retro evidence (R-REL-032-07 v1 was single-threshold conflation): aget core release body's correct calibration is depth-driven (8904 bytes acceptable for v3.16.0 sleeping-CAPs disclosure; would be wrong for trivial mechanical release). Two-perspective split surfaced 2026-05-02 by principal recalibration. Companion: `aget/sops/templates/RELEASE_BODY_TEMPLATE_core.md`. |
| R-REL-032-07b | ubiquitous | The validator SHALL verify, post-release, that each **template-{archetype}-aget** GitHub Release body content meets **alignment-driven** norms: floor 200 bytes for trivial alignment releases; floor 400 bytes when archetype-specific changes shipped; soft cap 1000 (above 1000 = WARN, indicates likely core-content inflation per anti-pattern); body SHALL be L909-sanitized; body SHOULD construct per `aget/sops/templates/RELEASE_BODY_TEMPLATE_template.md` skeleton selection. | v3.16.0 retro evidence (R-REL-032-07 v1): template release bodies' correct calibration is alignment-driven, NOT depth-driven. Inverse-of-core invariant: a 9000-byte aget core release body SHALL NOT pull templates above their alignment-norm (200-1000). v3.16.0 archetype-templates at 1155 bytes are calibration-over (correct floor; core-pattern crept into template body via verbatim CHANGELOG paste — the anti-pattern documented in template). |
| R-REL-032-07c | conditional | IF a release's aget core body exceeds 5000 bytes AND any template body exceeds 1000 bytes simultaneously, THEN the validator SHALL FLAG "verbose-core-leaked-into-template" pattern (alignment-driven discipline broken; templates picked up core-perspective content). | Inverse-of-core invariant codification. v3.16.0 cycle did NOT trigger this (8904 core + 1155 templates is below the joint-threshold) but the pattern is the failure mode the template-perspective is designed to prevent. |
