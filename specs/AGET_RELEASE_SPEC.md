...
| R-REL-032-07 | ubiquitous | The validator SHALL verify, post-release, that each released repo's GitHub Release body content is non-trivial (≥ 500 bytes for non-trivial releases) AND L909-sanitized (no private-* / gmelli/ / fleet-size / FLEET-* / SESSION_* / private-agent-name leaks). Release body SHOULD source from the repo's CHANGELOG entry per SOP_release_process v1.31 Phase 6.4.5.3. | v3.16.0 retro evidence: 14/14 repos shipped 138-byte minimal redirect bodies because `gh release create --notes "..."` was used inline rather than `--notes-file <CHANGELOG-extract>`. L671-at-GitHub-Release-surface instance. Closes Definition of Done "Discoverable" gap at the release-page level. |
